  ...
  build(parent) {
    return {
      ...
      sentence: (inWordCount) => {
        const sentence = parent.lorem.words(
          inWordCount || parent.random.number({ min: 3, max: 10 }),
        );
      ...
      sentences: (inSentenceCount, inSeparator) =>
        [...new Array(inSentenceCount || parent.random.number({ min: 2, max: 6 }))]
      ...
      paragraph: (inSentenceCount) =>
        parent.lorem.sentences((inSentenceCount || 3) + parent.random.number(3)),
